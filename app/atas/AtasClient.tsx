...
export default function AtasClient() {
  ...
  const fetchAtas = useCallback(async () => {
    try {
      ...
      setAtas(data)
      // Auto-open year/month of most recent ata
      if (data.length > 0) {
  ...
  return (
    <div style={{ display: 'flex', gap: 0, height: '100%', minHeight: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 16px 12px', borderBottom: '1px solid #F0F4FA', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontWeight: 700, fontSize: 15, color: '#1A2340' }}>Atas</span>
          {isGestorOrAdmin && (
            <button
              onClick={() => setShowModal(true)}
              style={{ padding: '5px 14px', borderRadius: 8, border: 'none', background: '#5B8DEF', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
            >
              + Nova
            </button>
          )}
                  ...
                  {openMonths.has(`${year}-${month}`) && mAtas.map(a => (
                    <button
                      ...
                      style={{
                        ...
                        border: 'none', background: selectedId === a.id ? '#EBF0FB' : 'none',
                        cursor: 'pointer', fontSize: 12, color: selectedId === a.id ? '#2A4F96' : '#334155',
                        borderLeft: selectedId === a.id ? '3px solid #5B8DEF' : '3px solid transparent',
                      }}
                    >
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
                ...
                {isGestorOrAdmin && (
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                    ...
                    <button
                      onClick={() => setEditingAta(selected)}
                      style={{ padding: '6px 16px', borderRadius: 8, border: '1px solid #5B8DEF', background: '#fff', color: '#5B8DEF', fontSize: 13, cursor: 'pointer' }}
                    >
                      Editar
                    </button>
                    <button
                      onClick={() => handleDelete(selected.id)}
                      style={{ padding: '6px 16px', borderRadius: 8, border: '1px solid #EF4444', background: '#fff', color: '#EF4444', fontSize: 13, cursor: 'pointer' }}
                    >
                      Excluir
                    </button>
                  </div>
      ...
      {/* ── Modals ── */}
      {showModal && (
        <AtaFormModal onSave={handleCreate} onClose={() => setShowModal(false)} />
      )}
      {editingAta && (
        <AtaFormModal
